# AngularJS - Dropdown Component

- AngularJS Dropdown is a lightweight component and includes all in one UI for selection, autocomplete and multiselection.
...
### Step 1: Install `angular-weblineindia-dropdown`:

#### NPM

```shell
npm install --save angular-weblineindia-dropdown
```
...
```js
import { NgSelectModule } from "angular-weblineindia-dropdown";
...
In your systemjs config file, `map` needs to tell the System loader where to look for `angular-weblineindia-dropdown`:

```js
map: {
  'angular-weblineindia-dropdown': 'node_modules/angular-weblineindia-dropdown/bundles/ng-select.umd.js',
}
```
...
angular-weblineindia-dropdown allows to provide custom selection implementation using `SELECTION_MODEL_FACTORY`. To override [default](https://github.com/ng-select/ng-select/blob/master/src/ng-select/lib/selection-model.ts) logic provide your factory method in your angular module.
...
angular-weblineindia-dropdown component implements `OnPush` change detection which means the dirty checking checks for immutable data types. That means if you do object mutations like:
...
angular-weblineindia-dropdown is inspired by [@ng-select/ng-select](https://www.npmjs.com/package/@ng-select/ng-select).

## License

[MIT](LICENSE)

[mit]: https://github.com/weblineindia/AngularJS-Dropdown/blob/master/LICENSE

## Keywords

angularjs-dropdown, angular-select, ui-select, angular-ui-select, angularjs-select, angular-dropdown, dropdown-component, angular-multiselect, multiselect, autocomplete, angular-autocomplete
